...
(() => {
  ...
  // ── Save-safety additions ───────────────────────────────────────────────
  // The base component swallows write failures silently and lets the page
  // reload while a whole-file write is still in flight — both cause dropped
  // uploads. These helpers surface failures and hold navigation until the
  // sidecar has actually committed.
  let pendingSaves = 0;       // writes started but not yet resolved
  function showToast(msg, isError) {
    let el = document.getElementById('om-slot-toast');
    if (!el) {
      el = document.createElement('div');
      el.id = 'om-slot-toast';
      el.style.cssText =
        'position:fixed;left:50%;bottom:22px;transform:translateX(-50%);z-index:2147483647;' +
        'max-width:min(520px,90vw);padding:11px 16px;border-radius:10px;' +
        'font:13px/1.4 system-ui,-apple-system,sans-serif;color:#fff;' +
        'box-shadow:0 10px 30px -8px rgba(0,0,0,.4);pointer-events:none;' +
        'opacity:0;transition:opacity .2s;text-align:center';
      document.body.appendChild(el);
    }
    el.style.background = isError ? '#b3261e' : '#1f7a44';
    el.textContent = msg;
    el.style.opacity = '1';
    clearTimeout(el._t);
    el._t = setTimeout(() => { el.style.opacity = '0'; }, isError ? 6000 : 2200);
  }
  // Hold reload / navigation while an upload write is unfinished so a
  // quick refresh can't race the commit and lose the image.
  window.addEventListener('beforeunload', (e) => {
    if (pendingSaves > 0 || saving || saveDirty) {
      e.preventDefault();
      e.returnValue = '';
      return '';
    }
  ...
  function save() {
    ...
    Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
      .then(() => { saving = false; if (saveDirty) { saveDirty = false; save(); }
        else showToast('照片已儲存', false); })
      .catch(() => {
        saving = false;
        showToast('照片儲存失敗，請重試一次（先別刷新頁面）。', true);
      })
      .then(() => { pendingSaves = Math.max(0, pendingSaves - 1); });
  }
